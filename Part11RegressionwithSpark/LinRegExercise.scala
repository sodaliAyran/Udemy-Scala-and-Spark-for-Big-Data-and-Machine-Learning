...
//val dataValid = data.select($"Avg Session Length", $"Time on App", $"Time on Website", $"Length of Membership", $"Yearly Amount Spent")
//dataValid.printSchema
val dataNoNA = data.na.drop()
// Print out an example Row
// Various ways to do this, just
// choose whichever way you prefer
val colnames = dataNoNA.columns
val firstrow = dataNoNA.head(1)(0)
println("\n")
...
val df = (dataNoNA.select(dataNoNA("Yearly Amount Spent").as("label"),
