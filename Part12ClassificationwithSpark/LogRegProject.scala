...
val datalabelled = data.select(data("Clicked on Ad").as("label"), $"Daily Time Spent on Site", $"Age", $"Area Income", $"Daily Internet Usage", $"Timestamp", $"Male")
val dataall = datalabelled.withColumn("Hour", hour($"Timestamp"))
...
val Array(training, test) = dataall.randomSplit(Array(0.7, 0.3), seed=42)
