...
val feature_data = assembler.transform(data).select($"features")
...
val scalerModel = scaler.fit(feature_data)

// Normalize each feature to have unit standard deviation.
// Use transform() off of this scalerModel object to create your scaledData
val scaledData = scalerModel.transform(normalized_data).select($"scaledFeatures")
